myApp.controller("themVideoController", function ($scope, $http, $window, $location, $filter,$sce, Url) {
    $scope.video = {};
    ...
    //Lấy idVideo từ Url
    $scope.currentIdVideo = Url.getParameterByName('idVideo');

    //Nếu sửa thì trả về giá trị của Video
    if ($scope.currentIdVideo) {
        $http.get('/API/VideosAPI/' + $scope.currentIdVideo)
