myApp.controller("themBannerController", function ($scope, $http, $window, $location, $filter, Url) {
    $scope.banner = {};
    ...
    //Lấy idBanner từ Url
    $scope.currentIdBanner = Url.getParameterByName('idBanner');

    //Nếu sửa thì trả về giá trị của Banner
    if ($scope.currentIdBanner) {
        $http.get('/API/BannersAPI/' + $scope.currentIdBanner)
    ...
    $scope.saveBanner = function () {
        if ($scope.currentIdBanner) {
            $http.put('/API/BannersAPI/' + $scope.banner.id, $scope.banner)
            .success(function () {
