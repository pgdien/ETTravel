myApp.controller("danhsachCategoryProductController", function ($scope, $http, $window, CategoryProduct, MenuMultiLevel, uiGridConstants) {
    $scope.category = {};
    $scope.categories = [];
    $scope.gridOptions = {};

    //Lấy danh sách Category
    $http.get('/API/CategoryProductsAPI/').success(function (data) { $scope.gridOptions.data = CategoryProduct.getTable(data); });
    ...
    $scope.EditCategory = function (idCategory) {
        $window.location.href = '/Admin/CategoryProducts/Create?idCategory=' + idCategory;
    }
    ...
    $scope.DeleteCategory = function (idCategory) {
        ...
        if (confirm("Bạn có chắc chắn muốn xóa?")) {
            ...
            $http.get('/API/CategoryProductsAPI/').success(function (data) {
                ...
                angular.forEach(categories, function (value, key) {
                    if (categories[key].idCategoryParent == idCategory) {
                        ...
                        $http.put('/API/CategoryProductsAPI/' + category.idCategory, category);
                    }
                });
            });

            //Xóa
            $http.delete('/API/CategoryProductsAPI/' + idCategory)
            .success(function () {
                $http.get('/API/CategoryProductsAPI').success(function (data) { $scope.gridOptions.data = CategoryProduct.getTable(data); });
