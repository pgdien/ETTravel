myApp.controller("danhsachOrderController", function ($scope, $http, $window, uiGridConstants) {
    $scope.Orders = [];
    $scope.gridOptions = {};

    //Lấy danh sách Post
    $http.get('/API/OrderAPI/').success(function (data) {
    ...
    [
        ...
        },
        {
            ...
            cellTemplate: '<div ><button style="margin-left: 10px; margin-top: 3px;" class="btn btn-xs btn-danger" ng-click="grid.appScope.DeleteOrder(row.entity.idOrder)"><span class="fa fa-bitbucket"></span></button></div>',
        }
    ];
