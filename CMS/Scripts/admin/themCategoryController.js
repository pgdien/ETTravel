...
myApp.controller("themCategoryController", function ($scope, $http, $window, $location,$filter, Alias, MenuMultiLevel, Url) {
    $scope.category = {};
    ...
    if ($scope.currentIdCategory) {
        $http.get('/API/CategoriesAPI/' + $scope.currentIdCategory)
    ...
    $http.get('/API/CategoriesAPI/') .success(function (data) {$scope.categoryParents = MenuMultiLevel.getDropdownMenu(data);});
    ...
    $scope.saveCategory = function () {
        ...
        if ($scope.currentIdCategory) {
            ...
            $http.put('/API/CategoriesAPI/' + $scope.category.idCategory, $scope.category)
            .success(function () {
                toastr.success('Thành công', 'Lưu Danh mục');
                $http.get('/API/CategoriesAPI/').success(function (data) { $scope.categoryParents = MenuMultiLevel.getDropdownMenu(data); $scope.$apply; });
            })
            .error(function () {
                toastr.error('Thất bại', 'Thêm Danh mục')
            });
        } else {
            $http.post('/API/CategoriesAPI/', $scope.category)
            .success(function () {
