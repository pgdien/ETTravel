myApp.controller("danhsachBannerController", function ($scope, $http, $window, uiGridConstants) {
    $scope.banners = [];
    $scope.gridOptions = {};

    //Lấy danh sách Post
    $http.get('/API/BannersAPI/').success(function (data) {
    ...
    $scope.EditBanner = function (id) {
        $window.location.href = '/Admin/Banners/Create?idBanner=' + id;
    }
    ...
    $scope.DeleteBanner = function (id) {
        ...
        if (confirm("Bạn có chắc chắn muốn xóa?")) {
            ...
            .success(function () {
                $http.get('/API/BannersAPI/').success(function (data) { $scope.gridOptions.data = data; });
